document.addEventListener('DOMContentLoaded', () => {
    ...
    if (e.key === 'Enter') {
      ...
      if (isInApplyForm || isInContactForm) {
          ...
          if (active && active.tagName === 'TEXTAREA') {
              // Allows shift+enter for new line in textarea
              if (e.shiftKey) {
                  return;
              } else {
                  // Treats Enter as submit for textarea without Shift
                  e.preventDefault(); 
  ...
  // --- 5. MODAL & APPLICATION FORM LOGIC ---
  ...
  function showConfirmModal(type, data) {
    ...
    if(confirmOverlay) {
        // Use a higher z-index for the confirmation overlay
  ...
  if (confirmProceedBtn) {
    confirmProceedBtn.addEventListener('click', () => {
      ...
      // 3. trigger actual emailjs and server logic
      if (type && data) {
  ...
  // --- UNIVERSAL SUBMISSION HANDLER (NEW) ---
  function handleFormSubmission(type, data) {
    ...
    // Form configuration and Server/EmailJS setup
    if (type === 'apply') {
        ...
        serverPromise = fetch('send_application.php', {
          method: 'POST',
        ...
        .then(r => r.json())
        .then(dbRes => {
          if (!dbRes || dbRes.success !== true) throw new Error(dbRes && dbRes.message ? dbRes.message : 'DB error');
          return true; // Indicate success for server side
        });
        ...
        serverPromise = fetch('send_inquiry.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(dbData)
        })
    ...
    // Wait for both server persistence AND email to complete
    Promise.all([serverPromise, emailPromise])
      .then(() => {
        ...
        if (type === 'apply' && applyForm) applyForm.reset(); 
  ...
  // --- 6. FORM EVENT LISTENERS (MODIFIED) ---
  ...
  if (applyForm) {
    applyForm.addEventListener('submit', e => {
      ...
      // Trigger confirmation modal instead of direct submission
